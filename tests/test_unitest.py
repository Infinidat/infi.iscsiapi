...
class ISCSIapi_host_TestCase(TestCase):
    ...
    def test_iscsi_sw_linux(self):
        ...
        if linux_iscsi.is_iscsi_sw_installed():
            linux_iscsi._uninstall_iscsi_software_initiator()
            linux_iscsi.install_iscsi_software_initiator()
        else:
            linux_iscsi.install_iscsi_software_initiator()
        self.assertNotEqual(linux_iscsi.is_iscsi_sw_installed, True)

    def test_iscsi_sw_win(self):
        from unittest import SkipTest
        from infi.os_info import get_platform_string
        ...
        if win_iscsi.is_iscsi_sw_installed():
            win_iscsi._uninstall_iscsi_software_initiator()
            win_iscsi.install_iscsi_software_initiator()
        else:
            win_iscsi.install_iscsi_software_initiator()
        self.assertNotEqual(win_iscsi.is_iscsi_sw_installed, True)
    ...
    def test_login_logout_linux(self):
        ...
        for field in net_space.get_field('ips'):
            ...
            result =
            self.assertEquals(linux_iscsi.get_sessions(), result[0].get('iqn'))
